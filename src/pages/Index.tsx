...
const ITEMS_BY_ZONE: Record<string, string[]> = {
  'Бар': [
    ...
    'Посуда заполнена. Чистая, натёртая.',
    'Маркировки присутствуют на всей продукции бара',
    'Бар сохраняет этикетки и сертификацию на продукцию',
    'Соблюдается товарное соседство, товары хранятся верно: 5 сантиметров от пола, в закрытых контейнерах, все соусники, топпинги и другие ёмкости имеют крышки и др.',
    ...
    'Кофемашина чистая: отсутствие засохшего молока на форсунках, отсутствие внутри холдеров чёрных кофейных масел, чистота групп, отсутствие кофейных таблеток в холдерах',
    ...
    'Раковина чистая',
    'Сервис бар отдаётся без задержек: напитки ставятся на раздачу в течение 5-7 минут вне запары, и до 15 минут в запару',
    ...
    'Волосы чистые, причёска аккуратная. Ногти подстрижены и чистые. Борода аккуратно оформлена',
...
type SectionedItem = { text: string; section?: string };
...
const buildRunner = (zone: string, title: string): RunnerData => {
  if (ITEMS_WITH_SECTIONS[zone]) {
    return {
      title, zone,
      items: ITEMS_WITH_SECTIONS[zone].map((item, i) => ({ id: i + 1, text: item.text, section: item.section })),
    };
  }
